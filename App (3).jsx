    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, boxSizing: "border-box" }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ fontWeight: 800, fontSize: 26, color: C.text }}>Mis Finanzas</p>
          <p style={{ color: C.textMid, fontSize: 14, marginTop: 4 }}>Tu dinero, bajo control</p>
        </div>
        <Card>
          <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
            {[["login", "Iniciar sesión"], ["registro", "Registrarse"]].map(([val, label]) => (
              <button key={val} onClick={() => { setModo(val); setError(""); }}
                style={{ flex: 1, padding: "11px", borderRadius: 12, border: `2px solid ${modo === val ? C.accent : C.border}`, background: modo === val ? C.accentLight : C.white, color: modo === val ? C.accent : C.textMid, fontFamily: "inherit", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                {label}
              </button>
            ))}
          </div>
...
// ─── TAB INICIO ───────────────────────────────────────────────────────────────
function TabInicio({ transacciones, balances, setBalances, userId, historial }) {
  ...
  return (
    <div className="fade" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <Card style={{ background: `linear-gradient(135deg, ${C.accent} 0%, #1250c0 100%)`, border: "none" }}>
        ...
        <p style={{ color: "#fff", fontSize: 38, fontWeight: 800, lineHeight: 1 }}>{fmt(patrimonio)}</p>
        <p style={{ color: "#ffffffbb", fontSize: 13, marginTop: 6 }}>Total en cuentas: {fmt(total)}</p>
        <div style={{ display: "flex", marginTop: 16, borderTop: "1px solid #ffffff30", paddingTop: 14 }}>
          <div style={{ flex: 1 }}>
      ...
      <p style={{ fontWeight: 700, fontSize: 16, paddingLeft: 2 }}>¿Dónde está tu plata? <span style={{ fontSize: 11, color: C.textLight, fontWeight: 500 }}>Tocá para editar</span></p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <Card style={{ textAlign: "center", padding: 14 }}>
          <p style={{ color: C.textMid, fontSize: 12, marginBottom: 6 }}>Ahorrás por mes</p>
          <p style={{ fontSize: 20, fontWeight: 800, color: C.green }}>{fmt(ahorrando)}</p>
          <p style={{ color: C.textMid, fontSize: 11, marginTop: 4 }}>{ahorroPct}% de tus ingresos</p>
        </Card>
        <Card style={{ textAlign: "center", padding: 14 }}>
          ...
          <p style={{ color: C.textMid, fontSize: 11, marginTop: 4 }}>promedio este mes</p>
        </Card>
      </div>

      {(totalDeliv > 0 || suscripciones.length > 0 || prediccion > 0) && (
        <>
          <p style={{ fontWeight: 700, fontSize: 16, paddingLeft: 2 }}>⚠️ Cosas a tener en cuenta</p>
          {totalDeliv > 0 && (
            <Card style={{ padding: 14, background: C.goldLight, border: `1.5px solid ${C.gold}30` }}>
              <p style={{ fontSize: 13, fontWeight: 500 }}>🛵 Gastaste {fmt(totalDeliv)} en deliveries ({deliveries.length} pedidos).</p>
            </Card>
          )}
          {suscripciones.length > 0 && (
            <Card style={{ padding: 14, background: C.accentLight, border: `1.5px solid ${C.accent}30` }}>
              <p style={{ fontSize: 13, fontWeight: 500 }}>🔄 {suscripciones.length} suscripciones por {fmt(totalSusc)}/mes. ¿Usás todas?</p>
            </Card>
          )}
          {prediccion > 0 && (
            <Card style={{ padding: 14, background: C.redLight, border: `1.5px solid ${C.red}30` }}>
              <p style={{ fontSize: 13, fontWeight: 500 }}>📅 Si seguís así, gastarás {fmt(prediccion)} a fin de mes.</p>
            </Card>
          )}
        </>
      )}
      ...
      {historial.length > 0 && (
        <>
          <p style={{ fontWeight: 700, fontSize: 16, paddingLeft: 2 }}>📊 Historial mensual</p>
          {historial.map((h, i) => (
            <Card key={i} style={{ padding: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <p style={{ fontWeight: 700, fontSize: 14 }}>{h.mes}</p>
                <p style={{ fontSize: 12, color: h.ahorro >= 0 ? C.green : C.red, fontWeight: 700 }}>
                  {h.ahorro >= 0 ? "+" : ""}{fmt(h.ahorro)}
                </p>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <p style={{ fontSize: 12, color: C.textMid }}>Ingresos: <b style={{ color: C.green }}>{fmt(h.ingresos)}</b></p>
                <p style={{ fontSize: 12, color: C.textMid }}>Gastos: <b style={{ color: C.red }}>{fmt(h.gastos)}</b></p>
              </div>
            </Card>
          ))}
        </>
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 4 }}>
            {Object.entries(gastosCat).sort((a, b) => b[1] - a[1]).map(([cat, tot]) => (
              <div key={cat} style={{ minWidth: 100, background: C.white, borderRadius: 16, padding: 12, textAlign: "center", flexShrink: 0, boxShadow: "0 2px 8px #00000008", border: `1px solid ${C.border}` }}>
  ...
  return (
    <div className="fade" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Card key={i}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ color: C.textMid, fontSize: 12, marginTop: 2 }}>Tiempo estimado: <b style={{ color: C.text }}>{meses} meses</b></p>
              </div>
              <p style={{ fontSize: 22, fontWeight: 800, color: meta.color, flexShrink: 0 }}>{pct}%</p>
            </div>
      ...
      <Card style={{ background: C.accentLight, border: `1.5px solid ${C.accent}30` }}>
        ...
        <p style={{ fontSize: 13, color: C.text }}>Ahorrás <b>{fmt(ahorrando)}</b> por mes.</p>
        <p style={{ fontSize: 13, color: C.textMid, marginTop: 6 }}>En un año podrías acumular: <b style={{ color: C.accent }}>{fmt(ahorrando * 12)}</b></p>
      </Card>
    </div>
  );
}
...
function DetalleEspacio({ espacio, userId, onClose }) {
  ...
  const agregar = async () => {
    ...
    const { error } = await supabase.from("espacio_movimientos").insert({
    ...
    if (!error) { setDesc(""); setMonto(""); cargarMovimientos(); }
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: C.bg, zIndex: 300, overflowY: "auto", overflowX: "hidden" }}>
      <div style={{ maxWidth: 500, margin: "0 auto", paddingBottom: 40, width: "100%" }}>
        <div style={{ background: C.white, borderBottom: `2px solid ${C.border}`, padding: "16px 16px", position: "sticky", top: 0, zIndex: 10, display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: 10, background: C.bg, border: "none", cursor: "pointer", fontSize: 18, flexShrink: 0 }}>←</button>
          <div style={{ minWidth: 0 }}>
            <p style={{ fontWeight: 800, fontSize: 16, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{espacio.nombre}</p>
        ...
        <div style={{ padding: "14px", display: "flex", flexDirection: "column", gap: 14 }}>
          {espacio.tipo === "sueldo" ? (
            <Card style={{ background: `linear-gradient(135deg, ${C.gold} 0%, #b45309 100%)`, border: "none" }}>
              ...
              <div style={{ display: "flex", marginTop: 14, borderTop: "1px solid #ffffff30", paddingTop: 12 }}>
                <div style={{ flex: 1 }}>
                  <p style={{ color: "#ffffffaa", fontSize: 11 }}>Este mes</p>
                  <p style={{ color: "#fff", fontSize: 16, fontWeight: 800 }}>{fmt(totalMes)}</p>
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ color: "#ffffffaa", fontSize: 11 }}>Pagos este mes</p>
                  <p style={{ color: "#fff", fontSize: 16, fontWeight: 800 }}>{pagosMes.length}</p>
                </div>
              </div>
            </Card>
          ) : (
            <Card style={{ background: `linear-gradient(135deg, ${C.accent} 0%, #1250c0 100%)`, border: "none" }}>
              ...
              <div style={{ display: "flex", marginTop: 14, borderTop: "1px solid #ffffff30", paddingTop: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ color: "#fff", fontSize: 16, fontWeight: 800 }}>{fmt(miTotal)}</p>
                </div>
                <div style={{ flex: 1 }}>
          ...
          <Card style={{ background: C.accentLight, border: `1.5px solid ${C.accent}30` }}>
            <p style={{ fontWeight: 700, fontSize: 13, color: C.accent, marginBottom: 6 }}>🔗 Compartí este código</p>
            <p style={{ fontSize: 12, color: C.textMid }}>La otra persona entra → Compartido → Unirse → escribe:</p>
            <p style={{ fontSize: 26, fontWeight: 800, color: C.accent, textAlign: "center", marginTop: 10, letterSpacing: 4 }}>{espacio.codigo}</p>
          </Card>
...
function TabCompartido({ userId }) {
  ...
  const crearEspacio = async () => {
    ...
    try {
      ...
      if (error) { alert("Error al crear el espacio: " + error.message); setCreando(false); return; }
  ...
  const unirseEspacio = async () => {
    ...
    try {
      const { data: espacio, error: errEsp } = await supabase
        .from("espacios").select("*").eq("codigo", codigoUnirse.trim().toUpperCase()).single();
      ...
      const { data: yaEsta } = await supabase
        .from("espacio_miembros").select("id")
      ...
      const { error: errMiembro } = await supabase
        .from("espacio_miembros").insert({ espacio_id: espacio.id, user_id: userId, rol: "miembro" });

      if (errMiembro) { setMsgUnirse("Error al unirse: " + errMiembro.message); setUniendose(false); return; }

      setModalUnirse(false); setCodigoUnirse(""); setMsgUnirse("");
      await cargarEspacios();
    } catch (e) {
      setMsgUnirse("Error inesperado. Intentá de nuevo.");
    }
    setUniendose(false);
  };
    ...
    <div className="fade" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setModalUnirse(true)}
            style={{ padding: "9px 12px", borderRadius: 12, border: `2px solid ${C.accent}`, background: C.accentLight, color: C.accent, fontFamily: "inherit", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
            Unirse
          </button>
          <button onClick={() => setModalNuevo(true)}
            style={{ padding: "9px 12px", borderRadius: 12, border: "none", background: C.accent, color: "#fff", fontFamily: "inherit", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
            + Nuevo
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card key={e.id} style={{ cursor: "pointer" }} onClick={() => setModalDetalle(e)}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontWeight: 700, fontSize: 15, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.nombre}</p>
      ...
      {modalNuevo && (
        <div onClick={e => e.target === e.currentTarget && setModalNuevo(false)}
          ...
          <div style={{ background: C.white, borderRadius: "24px 24px 0 0", padding: 24, width: "100%", maxWidth: 500, animation: "fadeUp 0.3s ease", boxSizing: "border-box" }}>
        ...
        <div onClick={e => e.target === e.currentTarget && setModalUnirse(false)}
          ...
          <div style={{ background: C.white, borderRadius: "24px 24px 0 0", padding: 24, width: "100%", maxWidth: 500, animation: "fadeUp 0.3s ease", boxSizing: "border-box" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <button onClick={() => { setModalUnirse(false); setMsgUnirse(""); setCodigoUnirse(""); }}
                style={{ width: 32, height: 32, borderRadius: 10, background: C.bg, border: "none", cursor: "pointer", fontSize: 16 }}>✕</button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <input placeholder="Código del espacio (ej: ABC123)" value={codigoUnirse}
                onChange={e => setCodigoUnirse(e.target.value.toUpperCase())}
                style={{ textTransform: "uppercase", letterSpacing: 2, fontSize: 18, fontWeight: 700, textAlign: "center" }} />
              {msgUnirse && <p style={{ color: msgUnirse.includes("miembro") ? C.gold : C.red, fontSize: 13, fontWeight: 600 }}>{msgUnirse}</p>}
      ...
      {modalDetalle && (
        <DetalleEspacio espacio={modalDetalle} userId={userId}
...
export default function App() {
  ...
  const verificarCierreMes = useCallback(async () => {
    ...
    const { data: yaGuardado } = await supabase.from("historial_mensual")
      .select("id").eq("user_id", session.user.id).eq("mes", mesActual).maybeSingle();
    ...
    const ingresos = (txs || []).filter(t => t.tipo === "ingreso").reduce((a, b) => a + b.monto, 0);
    const gastos   = (txs || []).filter(t => t.tipo === "gasto").reduce((a, b) => a + b.monto, 0);

    const nombreMes = hoy.toLocaleString("es-AR", { month: "long", year: "numeric" });
    await supabase.from("historial_mensual").insert({
      user_id: session.user.id, mes: nombreMes,
      ingresos, gastos, ahorro: ingresos - gastos,
    });
    cargarHistorial();
...
Datos del usuario: Ingresos: ${fmt(ingresos)}, Gastos: ${fmt(gastos)}, Ahorrando: ${fmt(ahorrando)}/mes.
...
Respondé simple, claro y con emojis. Máximo 3 párrafos.`,
  ...
  return (
    <>
      <style>{css}</style>
      <div style={{ maxWidth: 500, margin: "0 auto", minHeight: "100vh", display: "flex", flexDirection: "column", background: C.bg, overflowX: "hidden" }}>

        {/* HEADER */}
        <div style={{ background: C.white, borderBottom: `2px solid ${C.border}`, padding: "14px 16px", position: "sticky", top: 0, zIndex: 50, width: "100%" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
            <div style={{ minWidth: 0 }}>
              ...
              <p style={{ fontWeight: 800, fontSize: 18, color: C.text, marginTop: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Hola, {userNombre} 👋</p>
            </div>
            <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
              ...
              <button onClick={cerrarSesion}
                style={{ height: 40, width: 40, borderRadius: 12, border: `2px solid ${C.border}`, background: C.white, cursor: "pointer", fontSize: 16 }} title="Cerrar sesión">🚪</button>
            </div>
          </div>
        </div>

        {/* CONTENIDO */}
        <div style={{ flex: 1, padding: "16px 14px 110px", overflowY: "auto", overflowX: "hidden", width: "100%" }}>
          {tab === "inicio"      && <TabInicio transacciones={transacciones} balances={balances} setBalances={setBalances} userId={session.user.id} historial={historial} />}
        ...
        {modalAdd && (
          <div onClick={e => e.target === e.currentTarget && setModalAdd(false)}
            style={{ position: "fixed", inset: 0, background: "#00000060", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
            <div style={{ background: C.white, borderRadius: "24px 24px 0 0", padding: 22, width: "100%", maxWidth: 500, animation: "fadeUp 0.3s ease", boxSizing: "border-box" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  {[["gasto", "💸 Gasto"], ["ingreso", "💰 Ingreso"]].map(([val, label]) => (
                    <button key={val} onClick={() => setNewTx({ ...newTx, tipo: val })}
                      style={{ flex: 1, padding: "12px 8px", border: `2px solid ${newTx.tipo === val ? (val === "gasto" ? C.red : C.green) : C.border}`, borderRadius: 14, background: newTx.tipo === val ? (val === "gasto" ? C.redLight : C.greenLight) : C.white, color: newTx.tipo === val ? (val === "gasto" ? C.red : C.green) : C.textMid, fontFamily: "inherit", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                      {label}
                    </button>
                  ))}
                </div>
        ...
        {modalAI && (
          <div onClick={e => e.target === e.currentTarget && setModalAI(false)}
            ...
            <div style={{ background: C.white, borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 500, margin: "0 auto", height: "80vh", display: "flex", flexDirection: "column" }}>
              <div style={{ padding: "18px 18px 14px", borderBottom: `2px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div ref={chatRef} style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                {aiLoading && (
                  <div style={{ display: "flex" }}>
                    <div style={{ padding: "12px 14px", borderRadius: "18px 18px 18px 4px", background: C.bg, animation: "pulse 1s infinite", fontSize: 16 }}>💭</div>
                  </div>
                )}
              </div>
              <div style={{ padding: "8px 12px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 8, overflowX: "auto" }}>
              ...
              <div style={{ padding: 12, borderTop: `2px solid ${C.border}`, display: "flex", gap: 8 }}>
                ...
                <button onClick={enviarAI}
                  style={{ padding: "0 16px", background: C.accent, color: "#fff", border: "none", borderRadius: 14, fontFamily: "inherit", fontWeight: 700, fontSize: 14, cursor: "pointer", opacity: aiLoading ? 0.6 : 1, flexShrink: 0 }}>
                  Enviar
                </button>
              </div>
            </div>
        ...
        {modalCompra && (
          <div onClick={e => e.target === e.currentTarget && (setModalCompra(false), setCompraRes(null))}
            style={{ position: "fixed", inset: 0, background: "#00000060", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
            <div style={{ background: C.white, borderRadius: "24px 24px 0 0", padding: 22, width: "100%", maxWidth: 500, animation: "fadeUp 0.3s ease", maxHeight: "90vh", overflowY: "auto", boxSizing: "border-box" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 14 }}>
                ...
                <button onClick={analizarCompra}
                  style={{ background: C.accent, color: "#fff", border: "none", borderRadius: 14, padding: "13px", fontSize: 15, fontFamily: "inherit", fontWeight: 700, cursor: "pointer", opacity: compraLoading ? 0.7 : 1 }}>
